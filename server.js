...
사용자의 한글 번역을 엄격히 평가하여 아래 형식으로 **순수 JSON만** 한국어로 응답하세요. 다른 텍스트, 공백, 설명은 절대 포함하지 마세요.
{
  "종합 평가": {
    "스코어": 0,
    "피드백": "..."
...
- **피드백**:
  - 최대 100자로 작성, 간결하되 구체적으로.
  - 오류의 원인(문법, 단어, 문맥 등)을 명확히 설명.
  - 올바른 번역 예시를 포함.
  - 예: "문법 오류: '오는토요' → '이번 토요일'. 자연스러운 표현 사용."
  - 빈칸 입력 시: "입력 없음. 예: '올바른 번역 예시'."
- **중요**: 순수 JSON 형식만 반환, 추가 텍스트 금지.
    `;
